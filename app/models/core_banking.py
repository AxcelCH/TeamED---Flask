...
class Cliente(db.Model):
    ...
    cod_cliente = db.Column('COD_CLIENTE', db.String(10), primary_key=True, nullable=False, comment='ID único interno (CIF)')
...
class Cuenta(db.Model):
    ...
    num_cuenta = db.Column('NUM_CUENTA', db.String(20), primary_key=True, nullable=False, comment='CCI o interna')
...
class Tarjeta(db.Model):
    ...
    num_tarjeta = db.Column('NUM_TARJETA', db.String(16), primary_key=True, nullable=False, comment='PAN enmascarado o token')
...
class Movimiento(db.Model):
    ...
    id_trx = db.Column('ID_TRX', db.String(26), primary_key=True, nullable=False, comment='Timestamp + Secuencia única')
    num_cuenta = db.Column('NUM_CUENTA', db.String(20), db.ForeignKey('CORE_CUENTAS.NUM_CUENTA'), nullable=False)
    num_tarjeta = db.Column('NUM_TARJETA', db.String(16), db.ForeignKey('CORE_TARJETAS.NUM_TARJETA'), nullable=True, comment='Null si es transferencia web')
    cuenta_destino_origen = db.Column('CUENTA_DESTINO_ORIGEN', db.String(20), comment='Si es Salida: A quién envío. Si es Entrada: Quién me envía.')
    ...
    moneda = db.Column('MONEDA', db.String(3), default='PEN')
    glosa_trx = db.Column('GLOSA_TRX', db.String(100), nullable=False, comment='Texto crudo para NLP (ej: STARBUCKS)')
    cod_canal = db.Column('COD_CANAL', db.String(4), comment='ATM, POS, APP, WEB')
    cod_comercio = db.Column('COD_COMERCIO', db.String(15))
    ubicacion_trx = db.Column('UBICACION_TRX', db.String(50))
    saldo_post_trx = db.Column('SALDO_POST_TRX', db.Numeric(15, 2), comment='Saldo remanente tras la operación')
